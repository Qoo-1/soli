const screen=document.querySelector("#screen"),nav=document.querySelector("#nav"),clock=document.querySelector("#clock");
let route="surface",activeChar=null;
const navItems=[["chat","Chat"],["world","World"],["life","Life"],["cabinet","Cabinet"]];
function icon(n){return `<div class="icon">${SoliIcons[n]}</div>`}
function time(){clock.textContent=new Date().toLocaleTimeString("zh-CN",{hour:"2-digit",minute:"2-digit",hour12:false})}time();setInterval(time,1000);
function esc(s){return String(s??"").replace(/[&<>"']/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[m]))}
function navRender(){nav.className="nav";nav.innerHTML=navItems.map(([r,n])=>`<button class="nav-btn ${route===r||route==="chatroom"&&r==="chat"?"active":""}" data-r="${r}">${icon(r)}<span>${n}</span></button>`).join("");nav.querySelectorAll("[data-r]").forEach(b=>b.onclick=()=>{route=b.dataset.r;render()})}
...
function chatroom(){const c=activeChar,a=SoliStore.state.chats[c.id]||[];return `<div class="chat"><div class="chathead"><button class="back" id="back">‹</button><div class="avatar">${esc(c.initial||c.name[0])}</div><div class="chatname"><b>${esc(c.name)}</b><span>夏日奇冰 · ${esc(c.personality.slice(0,22))}</span></div>${icon("cabinet")}</div><div class="messages" id="messages">${a.length?a.map(m=>`<div class="msg ${m.role==="user"?"user":"ai"}">${m.role==="assistant"?`<div class="meta">${esc(c.name)}</div>`:""}${esc(m.content)}</div>`).join(""):`<div class="empty">从一句很普通的话开始。</div>`}</div><div class="composer"><textarea id="input" class="textarea" rows="1" placeholder="说点什么……"></textarea><button class="send" id="send">↑</button></div></div>`}
function world(){const w=SoliStore.state.worlds[0];return `<div class="page">${head("World","世界与 Pages")}<div class="paper"><div class="eyebrow">CURRENT WORLD</div><h2 style="margin:8px 0 0;font-size:22px">${esc(w.name)}</h2><p>${esc(w.description)}</p></div><div class="section">Pages</div><div class="list">${w.entries.map((e,i)=>`<div class="paper" data-page="${e.id}" style="cursor:pointer"><div class="eyebrow">PAGE ${String(i+1).padStart(2,"0")}</div><h3 style="margin:7px 0">${esc(e.name)}</h3><div class="keywords">${e.keywords.map(k=>`<span class="tag">${esc(k)}</span>`).join("")}</div></div>`).join("")}</div></div>`}
function life(){return `<div class="page">${head("Life","Soli 里慢慢发生的一些日常",`<button class="fab" id="plant">${icon("plus")}</button>`)}<div class="card" style="min-height:230px;display:flex;align-items:center;justify-content:center;text-align:center"><div><div style="font-size:30px;letter-spacing:.25em;color:#74aeb7">SUMMER</div><div style="margin-top:12px">这里不需要一直发生什么。</div><div class="mini" style="margin-top:6px">留一点空白，也是一种生活。</div></div></div><div class="section">Garden</div><div class="list">${SoliStore.state.life.plants.length?SoliStore.state.life.plants.map(p=>`<div class="row"><div class="avatar">01</div><div class="rowmain"><b>${esc(p.name)}</b><span>正在慢慢生长 · 第 ${p.day} 天</span></div></div>`).join(""):`<div class="empty">还没有种下植物。</div>`}</div></div>`}
...
function settingDetail(type){let title=type[0].toUpperCase()+type.slice(1);if(type==="ai")return `<div class="section">${title}</div><div class="card form"><label class="label">API Base URL</label><input id="base" class="input" value="${esc(SoliStore.state.api.baseUrl)}"><label class="label">API Key</label><input id="key" class="input" type="password" value="${esc(SoliStore.state.api.key)}" placeholder="保存于本机浏览器"><label class="label">Model</label><input id="model" class="input" value="${esc(SoliStore.state.api.model)}"><div class="grid2"><input id="temp" class="input" type="number" step=".1" value="${SoliStore.state.api.temperature}"><input id="tokens" class="input" type="number" value="${SoliStore.state.api.maxTokens}"></div><div class="actions"><button class="btn primary" id="saveAI">Save</button></div></div>`;
if(type==="voice")return `<div class="section">Voice</div><div class="card"><div class="mini">语音服务的 UI 与 provider 接口预留在这里。基础浏览器语音可以作为第一阶段 fallback。</div></div>`;
if(type==="chat")return `<div class="section">Chat</div><div class="card form"><label><input id="memory" type="checkbox" ${SoliStore.state.settings.autoMemory?"checked":""}> 自动保存轻量 Memory</label><div class="mini">Memory 与 World Pages 分离，分别记录“我们经历过什么”和“这个世界是什么”。</div></div>`;
if(type==="appearance")return `<div class="section">Appearance</div><div class="card"><b>Summer Ice</b><div class="mini" style="margin-top:5px">当前全局风格：夏日奇冰。界面材质使用冰白、雾蓝、透明玻璃与纸张层。</div></div>`;
...
document.querySelector("#saveAI")?.addEventListener("click",()=>{const s=SoliStore.state;s.api.baseUrl=base.value.trim();s.api.key=key.value.trim();s.api.model=model.value.trim();s.api.temperature=temp.value;s.api.maxTokens=tokens.value;SoliStore.save()});
...
async function send(){const input=document.querySelector("#input"),text=input.value.trim();if(!text)return;const c=activeChar,a=SoliStore.state.chats[c.id] ||= [];a.push({role:"user",content:text});SoliStore.save();input.value="";render();const box=document.querySelector("#messages");box.insertAdjacentHTML("beforeend",`<div class="msg ai" id="typing"><div class="meta">${esc(c.name)}</div>正在想……</div>`);box.scrollTop=box.scrollHeight;try{const answer=await SoliEngine.reply(c,a,text);a.push({role:"assistant",content:answer});SoliEngine.remember(c.id,text)}catch(e){a.push({role:"assistant",content:"AI 连接失败：\n"+e.message)}SoliStore.save()}render()}
function render(){navRender();screen.innerHTML=route==="surface"?surface():route==="chat"?chats():route==="chatroom"?chatroom():route==="world"?world():route==="life"?life():cabinet();bind();if(route==="chatroom"){const b=document.querySelector("#messages");b.scrollTop=b.scrollHeight}}
render();
